...
此数值越大，采样精确程度就越高，得到的数据就越精确。但是，调大该数值会很大影响到雪绘的生成时间;减小该数值对于雪绘的生成时间的降低不明显，只要不低于10一般会不对结果产生较大影响。
...
1. 低rat玩家(12.2左右)若对自己的推分建议感到不满意，或者发现雪绘推了自己不会的歌，可以适当减小x的值(但是一般会出现nan的情况，下面有讲)
